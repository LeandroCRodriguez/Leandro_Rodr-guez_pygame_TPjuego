...
class Auxiliar:
    ...
    @staticmethod
    def getSurfaceFromSeparateFiles(path_format, from_index, quantity, flip=False, step=1, scale=1, w=0, h=0, repeat_frame=1, enlarge=False):
        lista = []
        for i in range(from_index, quantity + from_index):
            path = path_format.format(i)
            surface_fotograma = pygame.image.load(path)
            if enlarge:
                surface_fotograma = pygame.transform.scale(surface_fotograma, (surface_fotograma.get_width() * 0.3, surface_fotograma.get_height() * 0.3)).convert_alpha()
            else:
                fotograma_ancho_scaled = int(surface_fotograma.get_rect().w * scale)
                fotograma_alto_scaled = int(surface_fotograma.get_rect().h * scale)
                if (scale == 1 and w != 0 and h != 0):
